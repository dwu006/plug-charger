...
class IOSPhone(BasePhone, Teleoperator):
    ...
    def connect(self) -> None:
        ...
        lookup = hebi.Lookup()
        time.sleep(2.0)
        family = getattr(self.config, 'family', 'HEBI')
        name = getattr(self.config, 'name', 'mobileIO')
        print(f"Looking for Mobile I/O with family='{family}', name='{name}'")
        
        print("Scanning for available HEBI devices...")
        time.sleep(1.0)
        entry_list = lookup.entrylist
        num_devices = entry_list._size
        print(f"Found {num_devices} HEBI device(s) on network:")
        for i in range(num_devices):
            entry = entry_list[i]
            print(f"  Device {i}: family='{entry.family}', name='{entry.name}'")
        
        group = lookup.get_group_from_names([family], [name])
        if group is None:
            raise RuntimeError(f"Mobile I/O not found — looking for family='{family}', name='{name}'. Check that your Mobile I/O app has these exact settings and is on the same Wi-Fi network.")
        self._group = group
    ...
    def calibrate(self) -> None:
        ...
        print("Press and hold ANY button (B1-B8) in the HEBI Mobile I/O app to capture this pose...\n")
    ...
    def _wait_for_capture_trigger(self) -> tuple[np.ndarray, Rotation]:
        ...
        """
        print("Waiting for B1 button press...")
        check_count = 0
        while True:
            has_pose, position, rotation, fb_pose = self._read_current_pose()
            if not has_pose:
                if check_count % 100 == 0:
                    print("  No pose data yet, waiting...")
                time.sleep(0.01)
                check_count += 1
                continue
            ...
            if check_count % 100 == 0:
                if any_button_pressed:
                    print(f"  Button B{pressed_button} is pressed! Use this button for calibration.")
                else:
                    print(f"  Waiting for any button press (B1-B8)...")
            
            if any_button_pressed:
                print(f"Button B{pressed_button} detected! Capturing pose...")
